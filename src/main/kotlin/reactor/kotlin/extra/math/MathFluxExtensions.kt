 ...
 * @since 1.0.0
 */
fun <T : Any> Flux<T>.min(comp: Comparator<T>): Mono<T> =
    MathFlux.min(this, comp)
...
fun <T : Any> Flux<T>.min(comp: (T, T) -> Int): Mono<T> =
    MathFlux.min(this, Comparator(comp))
 ...
 * @since 1.0.0
 */
fun <T : Any> Flux<T>.max(comp: Comparator<T>): Mono<T> =
    MathFlux.max(this, comp)
...
fun <T : Any> Flux<T>.max(comp: (T, T) -> Int): Mono<T> =
    MathFlux.max(this, Comparator(comp))